...
# Environment configuration is now handled by gen_env_config.sh
# No need for force_fix_env_config.sh anymore
...
# Environment configuration already generated by gen_env_config.sh
...
# DYNAMIC FIREBASE SETUP: Configure Firebase based on PUSH_NOTIFY and IS_GOOGLE_AUTH
# NOTE: This runs AFTER app name and package name changes are complete
log "🔥 Setting up dynamic Firebase configuration..."
if [ -f "lib/scripts/android/dynamic_firebase_setup.sh" ]; then
    chmod +x lib/scripts/android/dynamic_firebase_setup.sh
    if lib/scripts/android/dynamic_firebase_setup.sh; then
        log "✅ Dynamic Firebase configuration completed"
    else
        log "⚠️ Dynamic Firebase configuration completed with warnings"
    fi
else
    log "❌ Dynamic Firebase setup script not found"
    log "🔄 Using legacy Firebase setup..."
...
if [ "${WORKFLOW_ID:-}" = "android-free" ]; then
    ...
    if [ "${PUSH_NOTIFY:-}" = "true" ]; then
        ...
        # Run dynamic Firebase configuration fix first
        if [ -f "lib/scripts/android/fix_firebase_dynamic.sh" ]; then
            log "🔧 Running dynamic Firebase configuration fix..."
            chmod +x lib/scripts/android/fix_firebase_dynamic.sh
            if lib/scripts/android/fix_firebase_dynamic.sh; then
                log "✅ Dynamic Firebase configuration fix completed"
            else
                log "⚠️ Dynamic Firebase configuration fix failed, continuing with fallback..."
            fi
        fi
        ...
        if [ -n "${FIREBASE_CONFIG_ANDROID:-}" ]; then
            ...
            if [ -f "lib/scripts/android/firebase.sh" ]; then
                ...
                if lib/scripts/android/firebase.sh; then
                    ...
                    log "❌ Android Firebase configuration failed"
                    log "🔄 Continuing with fallback Firebase configuration..."
                fi
            else
                log "❌ Android Firebase script not found"
                log "🔄 Continuing with fallback Firebase configuration..."
            ...
            # The dynamic fix script will handle this
        fi
    else
...
if [ -f "lib/config/env_config.dart" ]; then
    ...
    log "✅ Environment configuration verified - branch variable removed as not required"
else
...
if [ "$APK_FOUND" = false ]; then
    ...
    # Find any APK file and add it to the paths
    FOUND_APK=$(find build -name "*.apk" 2>/dev/null | head -1)
    if [ -n "$FOUND_APK" ]; then
        log "   Found APK: $FOUND_APK"
        APK_PATHS+=("$FOUND_APK")
        APK_FOUND=true
        log "✅ Added found APK to verification paths"
    else
        log "   No APK files found in build directory"
    fi
fi

# Verify APK package name if found
if [ "$APK_FOUND" = true ] && command -v aapt >/dev/null 2>&1; then
    log "🔧 aapt tool found, version: $(aapt version 2>/dev/null || echo 'unknown')"
    log "📦 ===== APK PACKAGE VERIFICATION ====="
    for apk_path in "${APK_PATHS[@]}"; do
        if [ -f "$apk_path" ]; then
            # Extract package information using multiple parsing methods for reliability
            log "🔍 Raw aapt output for package:"
            aapt dump badging "$apk_path" 2>/dev/null | grep "package:" | head -1 | while read -r line; do
                log "   $line"
            done
            
            # Try multiple parsing methods
            APK_PACKAGE=$(aapt dump badging "$apk_path" 2>/dev/null | grep "package:" | head -1 | awk -F"'" '{print $2}' || echo "UNKNOWN")
            
            # If awk fails, try cut
            if [ "$APK_PACKAGE" = "UNKNOWN" ] || [ -z "$APK_PACKAGE" ]; then
                APK_PACKAGE=$(aapt dump badging "$apk_path" 2>/dev/null | grep "package:" | head -1 | cut -d"'" -f2 || echo "UNKNOWN")
            fi
            
            # If cut fails, try sed
            if [ "$APK_PACKAGE" = "UNKNOWN" ] || [ -z "$APK_PACKAGE" ]; then
                APK_PACKAGE=$(aapt dump badging "$apk_path" 2>/dev/null | grep "package:" | head -1 | sed "s/.*name='\([^']*\)'.*/\1/" || echo "UNKNOWN")
            fi
            
            # Extract version name and code with proper parsing
            APK_VERSION_NAME=$(aapt dump badging "$apk_path" 2>/dev/null | grep "versionName" | head -1 | sed -n "s/.*versionName='\([^']*\)'.*/\1/p" || echo "UNKNOWN")
            APK_VERSION_CODE=$(aapt dump badging "$apk_path" 2>/dev/null | grep "versionCode" | head -1 | sed -n "s/.*versionCode='\([^']*\)'.*/\1/p" || echo "UNKNOWN")
            
            # If sed fails, try alternative parsing
            if [ "$APK_VERSION_NAME" = "UNKNOWN" ] || [ -z "$APK_VERSION_NAME" ]; then
                APK_VERSION_NAME=$(aapt dump badging "$apk_path" 2>/dev/null | grep "versionName" | head -1 | cut -d"'" -f2 || echo "UNKNOWN")
            fi
            
            if [ "$APK_VERSION_CODE" = "UNKNOWN" ] || [ -z "$APK_VERSION_CODE" ]; then
                APK_VERSION_CODE=$(aapt dump badging "$apk_path" 2>/dev/null | grep "versionCode" | head -1 | cut -d"'" -f2 || echo "UNKNOWN")
            fi
            ...
            # Validate package name before comparison
            if [ "$APK_PACKAGE" = "UNKNOWN" ] || [ -z "$APK_PACKAGE" ] || [[ "$APK_PACKAGE" =~ ^[0-9]+$ ]]; then
                log "⚠️ Invalid package name extracted: '$APK_PACKAGE'"
                log "🔍 Package name validation failed - this indicates a parsing issue"
                log "🔍 Expected format: com.example.app"
                log "🔍 Got: '$APK_PACKAGE'"
                
                # Set a fallback package name for comparison
                APK_PACKAGE="PARSING_FAILED"
            fi
