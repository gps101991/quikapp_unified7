  ...
  try {
    ...
    if (Platform.isIOS) {
      ...
      if (iosPlugin != null) {
        ...
        debugPrint('🍎 iOS permission request completed');
        ...
        debugPrint('❌ iOS notification plugin not available');
        return false;
      }
    ...
    try {
      ...
      if (apnsToken == null) {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          if (apnsToken != null) {
            debugPrint(
                '✅ APNS Token received after ${i + 1} attempts: $apnsToken');
            break;
          }
        }
      }
      ...
      if (apnsToken == null) {
        debugPrint(
            '⚠️ APNS token not available after waiting, but continuing with FCM setup...');
        debugPrint(
            '💡 This is normal on first app launch or when push notifications are not yet authorized');
      } else {
      ...
      debugPrint(
          '💡 This error is usually temporary and will resolve on retry');
    }
  }
